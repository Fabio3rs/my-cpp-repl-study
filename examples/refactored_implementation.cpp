...
class ReplContext : public interfaces::IReplContext {
  ...
  public:
    ...
    void removeLinkLibrary(const std::string &library) override {
        std::unique_lock lock(mutex_);
        config_.link_libraries.erase(std::remove(config_.link_libraries.begin(),
...
class ClangCompiler : public interfaces::ICompiler {
  private:
    ...
    runCommand(const std::string &command, std::chrono::milliseconds timeout) {
        // For simplicity, using system() - in production, use proper process
        // management
        std::string temp_output =
            temp_dir_ / ("compile_output_" +
                         std::to_string(compilation_counter_++) + ".txt");
  ...
  public:
    ...
    compile(const std::string &source_code,
            const interfaces::CompilerConfig &config) override {
        try {
            auto start_time = std::chrono::steady_clock::now();
            ...
            auto source_file =
                temp_dir_ / ("source_" + std::to_string(counter) + ".cpp");
            auto output_file =
                temp_dir_ / ("lib_" + std::to_string(counter) + ".so");
            ...
            // Write source code to file
            {
                ...
                if (!file) {
                    return std::unexpected(
                        interfaces::CompilerError::FileNotFound);
                }
                file << source_code;
            }
            ...
            std::string compile_cmd =
                buildCompileCommand(config, source_file, output_file);
            ...
            if (!std::filesystem::exists(output_file)) {
                return std::unexpected(
                    interfaces::CompilerError::InternalError);
            }
    ...
    validateSyntax(const std::string &source_code) const override {
        ...
        auto temp_source =
            temp_dir_ /
            ("syntax_check_" + std::to_string(compilation_counter_++) + ".cpp");
        ...
        std::string check_cmd =
            "clang++ -fsyntax-only " + temp_source.string() + " 2>&1";
  ...
  public:
    ...
    execute(const interfaces::CompilationResult &compilation_result) override {
        try {
            ...
            auto library = std::make_unique<ScopedLibrary>(
                compilation_result.library_path);

            if (!library->isValid()) {
                return std::unexpected(
                    interfaces::ExecutionError::InvalidOperation);
            }
            ...
            if (!exec_fn.has_value()) {
                return std::unexpected(
                    interfaces::ExecutionError::SymbolNotFound);
            }
            ...
            try {
                ...
                return std::unexpected(
                    interfaces::ExecutionError::RuntimeException);
            }
    ...
    setVariable(const std::string &variable_name,
...
std::unique_ptr<IReplEngine>
createReplEngine(std::unique_ptr<ICompiler> compiler,
