...
public:
    BaseCommand(std::string name, std::string description, std::string usage)
        : name_(std::move(name)), description_(std::move(description)), usage_(std::move(usage)) {}
...
public:
    ...
    std::expected<void, ReplError> execute(const std::vector<std::string>& args,
        ...
        if (is_system_header) {
            ...
            std::filesystem::path header_dir = std::filesystem::path(clean_header).parent_path();
            if (!header_dir.empty()) {
...
private:
    std::reference_wrapper<const std::unordered_map<std::string, std::unique_ptr<ICommand>>> commands_ref_;

public:
    explicit HelpCommand(const std::unordered_map<std::string, std::unique_ptr<ICommand>>& commands)
