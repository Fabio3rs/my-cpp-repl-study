...
class MockCompiler : public ICompiler {
  public:
    MOCK_METHOD(std::expected<CompilationResult, CompilerError>, compile,
                (const std::string &source_code, const CompilerConfig &config),
                (override));
    MOCK_METHOD(bool, isAvailable, (), (const, override));
    MOCK_METHOD(std::expected<std::string, CompilerError>, getVersion, (),
                (const, override));
    MOCK_METHOD(std::expected<bool, CompilerError>, validateSyntax,
                (const std::string &source_code), (const, override));
};

class MockExecutor : public IExecutor {
  public:
    MOCK_METHOD(std::expected<ExecutionResult, ExecutionError>, execute,
                (const CompilationResult &compilation_result), (override));
    MOCK_METHOD(std::expected<std::any, ExecutionError>, getVariable,
                (const std::string &variable_name), (override));
    MOCK_METHOD(std::expected<void, ExecutionError>, setVariable,
                (const std::string &variable_name, const std::any &value),
                (override));
    MOCK_METHOD(std::vector<std::string>, getAvailableVariables, (),
                (const, override));
...
class MockReplContext : public IReplContext {
  ...
  public:
    MOCK_METHOD(void, setCompilerConfig, (const CompilerConfig &config),
                (override));

    CompilerConfig getCompilerConfig() const override { return config_; }

    void addIncludeDirectory(const std::filesystem::path &path) override {
        include_dirs_.push_back(path);
    }

    MOCK_METHOD(void, removeIncludeDirectory,
    ...
    MOCK_METHOD(void, removeLinkLibrary, (const std::string &library),
                (override));
    ...
    MOCK_METHOD(void, removePreprocessorDefinition,
...
class CompilerTests : public ::testing::Test {
  protected:
    void SetUp() override { compiler_ = std::make_unique<MockCompiler>(); }
...
class ExecutorTests : public ::testing::Test {
  protected:
    void SetUp() override { executor_ = std::make_unique<MockExecutor>(); }
...
class ReplContextTests : public ::testing::Test {
  protected:
    void SetUp() override { context_ = std::make_unique<MockReplContext>(); }
...
TEST_F(ExecutorTests, GetNonExistentVariable) {
    ...
    EXPECT_CALL(*executor_, getVariable(var_name))
        .WillOnce(
            testing::Return(std::unexpected(ExecutionError::SymbolNotFound)));
...
TEST_F(CommandSystemTests, ParseCommandLine) {
    ...
    EXPECT_EQ(CommandParser::extractCommand(":include <iostream>"),
              "include <iostream>");
}
...
class ReplIntegrationTests : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        temp_dir_ =
            std::filesystem::temp_directory_path() / "repl_integration_test";
...
TEST_F(ReplIntegrationTests, EndToEndVariableDeclaration) {
    // Test the full pipeline: command parsing -> compilation -> execution ->
    // result retrieval
...
TEST_F(ReplIntegrationTests, CommandSystemIntegration) {
    ...
    ASSERT_TRUE(command_manager.executeCommand("include <iostream>", *context_)
                    .has_value());
    ASSERT_TRUE(
        command_manager.executeCommand("link pthread", *context_).has_value());
    ASSERT_TRUE(command_manager.executeCommand("define DEBUG 1", *context_)
                    .has_value());
...
class ReplPerformanceTests : public ::testing::Test {
  protected:
    void SetUp() override { context_ = createReplContext(); }
...
TEST(ReplUsageExample, BasicWorkflow) {
    ...
    ASSERT_TRUE(command_manager.executeCommand("include <iostream>", *context)
                    .has_value());
    ASSERT_TRUE(
        command_manager.executeCommand("link c++", *context).has_value());
